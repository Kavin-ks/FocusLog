...
/// Storage service that uses cached user data from AuthService.
/// Falls back to local storage for non-logged-in users (demo mode).
class StorageService {
  ...
  Future<void> saveEntries(DateTime date, List<TimeEntry> entries) async {
    ...
    final List<Map<String, dynamic>> jsonList = entries
        .map((entry) => entry.normalized().toJson())
        .toList();
  ...
  Future<String> exportAllAsCsv() async {
    ...
    buffer.writeln(
      'date,id,startTime,endTime,activityName,category,durationMinutes,energy,intent',
    );
  ...
  Future<void> clearAllData() async {
    ...
    for (final key in keys) {
      if (key.startsWith('time_entries_') ||
          key.startsWith('${_reflectionKey}_')) {
  ...
  Future<bool> isCategoryUsed(String categoryId) async {
    ...
    final keys = prefs
        .getKeys()
        .where((k) => k.startsWith('time_entries_'))
        .toList();
    for (final key in keys) {
      final entriesJson = prefs.getString(key);
      if (entriesJson == null) continue;
